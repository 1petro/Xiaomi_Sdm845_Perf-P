...
/*        data->reset = of_get_named_gpio_flags(np, "focaltech,reset-gpio", 0, &data->reset_gpio_flags);
        if (data->reset < 0)
                printk("Unable to get reset_gpio");
        ...
        if (gpio_is_valid(data->reset_gpio)) {
                ...
                ret = gpio_direction_output(data-->reset, 0);
                if (ret) {
...
static int __init fts_ts_init(void)
{
        ...
        if (ret != 0) {
                printk("Focaltech touch screen driver init failed!");
        }
        return ret;
}
